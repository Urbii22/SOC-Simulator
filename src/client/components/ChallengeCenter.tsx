...
function ModeCard({ icon, title, description, detail, disabled, onClick }: { icon: React.ReactNode; title: string; description: string; detail: string; disabled: boolean; onClick: () => void }) {
  return <button className="mode-card" disabled={disabled} onClick={onClick}><span>{icon}</span><em className="mode-tag">{title === 'Quick Challenge' ? 'Rápido' : title === 'Training Session' ? 'Aprendizaje' : 'Avanzado'}</em><strong>{title}</strong><p>{description}</p><small>{detail}</small><b>Iniciar<ChevronRight size={14} /></b></button>;
}
...
function HistoryView({ items, busy, onOpen }: { items: ChallengeHistoryItem[]; busy: boolean; onOpen: (item: ChallengeHistoryItem) => Promise<void> }) {
  ...
  return <div className="records-screen history-screen"><header><LabArtwork variant="history" /><History /><div><span className="eyebrow">Historial · Sesiones</span><h1>Historial de sesiones</h1><p>Revisa tus sesiones de entrenamiento, analiza tu progreso y continúa practicando.</p></div></header><div className="metric-strip history-metrics"><Metric label="Sesiones completadas" value={completed.length} /><Metric label="Rendimiento promedio" value={completed.length ? `${Math.round(completed.reduce((sum, item) => sum + item.accuracy, 0) / completed.length)}%` : '—'} /><Metric label="Tiempo promedio" value={completed.length ? duration(completed.reduce((sum, item) => sum + item.durationMs, 0) / completed.length) : '—'} /><Metric label="Sesiones totales" value={items.length} /></div>{latest && <section className="latest-session"><div><span className="eyebrow">Última sesión</span><h2>Sesión de entrenamiento</h2><p>{new Date(latest.createdAt).toLocaleString('es')} · {latest.mode} · <code>{latest.seed}</code></p></div><div><span>Progreso</span><progress aria-label="Progreso de la última sesión" value={latest.completed} max={latest.count} /><small>{latest.completed}/{latest.count} completado</small></div><div><span>Duración</span><strong><Clock3 size={18} />{duration(latest.durationMs)}</strong></div><button className="secondary-action" disabled={busy} onClick={() => void onOpen(latest)}>Abrir sesión <ChevronRight size={16} /></button></section>}{items.length ? <><div className="record-table"><div className="record-head"><span>Fecha</span><span>Modo / semilla</span><span>Progreso</span><span>Precisión</span><span>Duración</span><span>Acciones</span></div>{items.map((item) => <div className="record-row" key={item.id}><time>{new Date(item.createdAt).toLocaleString('es')}</time><span><strong>{item.mode}</strong><code>{item.seed}</code></span><span><span>{item.completed}/{item.count}</span><progress aria-label={`Progreso de ${item.seed}`} value={item.completed} max={item.count} /><small className={`session-status status-${item.status}`}>{item.status === 'completed' ? 'Completada' : 'En progreso'}</small></span><strong>{item.accuracy}%</strong><span>{duration(item.durationMs)}</span><button disabled={busy} onClick={() => void onOpen(item)}>Abrir<ChevronRight size={14} /></button></div>)}</div><p className="records-count">Mostrando {items.length} {items.length === 1 ? 'sesión' : 'sesiones'}</p></> : <p className="empty-records">Todavía no hay sesiones. El primer Quick Challenge aparecerá aquí.</p>}</div>;
}
...
function StatsView({ stats }: { stats: ChallengeStats | null }) {
  ...
  return <div className="records-screen stats-screen"><header><LabArtwork variant="stats" /><BarChart3 /><div><span className="eyebrow">Estadísticas · Rendimiento acumulado</span><h1>Estadísticas de entrenamiento</h1><p>Analiza tu rendimiento, identifica áreas de mejora y sigue tu progreso en el laboratorio SOC.</p></div></header><div className="metric-strip"><Metric label="Casos completados" value={stats.investigations} /><Metric label="Precisión media" value={`${stats.averageAccuracy}%`} /><Metric label="Veredictos correctos" value={`${stats.verdictAccuracy}%`} /><Metric label="Tiempo medio" value={duration(stats.averageTimeMs)} /><Metric label="Pistas utilizadas" value={stats.hintsUsed} /></div><div className="stats-grid"><Breakdown title="Rendimiento por dificultad" items={stats.byDifficulty} /><Breakdown title="Rendimiento por categoría" items={stats.byCategory} /><Breakdown title="Rendimiento por técnica MITRE ATT&CK" items={stats.byMitre} /><Breakdown title="Plantillas a reforzar" items={stats.weakestTemplates} /></div><section className="recommendations"><span className="eyebrow">Práctica recomendada</span>{stats.recommendations.map((item) => <p key={item}><Target size={14} />{item}</p>)}</section></div>;
}

function Metric({ label, value }: { label: string; value: string | number }) { const Icon = /Tiempo|Duración/.test(label) ? Clock3 : /Pistas/.test(label) ? Lightbulb : /Veredictos/.test(label) ? ShieldCheck : /Precisión|Rendimiento/.test(label) ? Target : BarChart3; return <div><i><Icon size={25} /></i><span>{label}</span><strong>{value}</strong></div>; }
function Breakdown({ title, items }: { title: string; items: ChallengeStats['byDifficulty'] }) { return <section><h3><BarChart3 size={20} />{title}</h3>{items.slice(0, 8).map((item) => <div key={item.key}><span>{item.key}</span><progress aria-label={`Precisión de ${item.key}`} value={item.accuracy} max="100" /><strong>{item.accuracy}%</strong><small>{item.investigations}</small></div>)}{!items.length && <p>Sin datos todavía.</p>}</section>; }
